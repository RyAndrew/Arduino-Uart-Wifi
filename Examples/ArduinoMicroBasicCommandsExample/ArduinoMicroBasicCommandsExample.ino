...
void setup() {
  ...
  //NOTE!
  //This is only for an Arduino Micro or ATmega32u4 which contains a dedicated serial separate from the USB Serial
  Serial1.begin(115200); // AT+UART=115200,8,1,None,NFC
}
...
boolean waitForSocketBToConnect(){
 ...
 char lastCmdResultChar;
  ...
  while( millis() - start < waitTimeout ){
    ...
    if (Serial1.available()) { //Output should be "+ok=on"
